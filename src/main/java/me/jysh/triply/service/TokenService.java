...
import static me.jysh.triply.constant.Constants.*;
...
public class TokenService {
    ...
    public RefreshTokenEntry validateRefreshToken(final String refreshToken) {
        final Optional<RefreshTokenEntity> optionalRefreshToken = refreshTokenRepository.findByToken(refreshToken);

        final RefreshTokenEntity token = optionalRefreshToken.orElseThrow(
                () ->
                        new TokenRefreshException(refreshToken, "Refresh token is invalid. Please use a valid refresh token.")
        );
